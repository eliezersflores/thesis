...
mtry_opt = 0.7 # optimized with the 'tuning.py' script. 
...
for fold_idx in range(1, 11):
    ...
    clf = RandomForestClassifier(n_estimators=200, max_features = mtry_opt, n_jobs=-1)  
    clf.fit(Ytrain, Ttrain.ravel())  
    
    preds = clf.predict_proba(Yvalid)
    perfs[str(fold_idx)] = roc_auc_score(Tvalid, preds[:,1])

perfs.to_csv(jnt(results_dir, 'RF.csv'), index=False, header=False)
