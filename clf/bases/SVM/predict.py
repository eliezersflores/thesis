...
C_opt = 2**-5 # optimized with the 'tuning.py' script. 
...
for fold_idx in range(1, 11):
    ...
    clf = SVC(kernel='linear', C=C_opt, probability=True)
    clf.fit(Ytrain, Ttrain.ravel())    
    
    preds = clf.predict_proba(Yvalid)
    perfs[str(fold_idx)] = roc_auc_score(Tvalid, preds[:,1])

perfs.to_csv(jnt(results_dir, 'SVM.csv'), index=False, header=False)
